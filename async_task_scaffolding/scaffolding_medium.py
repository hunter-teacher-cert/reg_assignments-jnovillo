...
def is_prime(num):    
    if isinstance(num, int):    
        if num > 1:
            # Make a for loop in a rage from 2 to num
...
def get_primes(x):
    if isinstance(x, int):    
	    ...
	    # After that, make a condition to return the list primes if its lenght is equal to x
            return primes
    else:
        print("Error: Parameter should be an integer")
        return None        
...
def get_max_prime_factor(num):
    if isinstance(num, int):    
        ...
        if isPrime:           
           ...
           # The following for loop is incorrect.
           # Please fix it to make the function work.
